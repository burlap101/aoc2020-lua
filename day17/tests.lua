...
---Determines if supplied coords are 4D
---@param coord Coordinates | Coordinates4D
---@return boolean
local function is4DCoord(coord)
	if coord.w then
		return true
	end
	return false
...
function TestSpace4DNew()
	...
	for _, cube in pairs(space.cubes) do
		...
		lu.assertIsTrue(is4DCoord(cube.coords))
	end
end

function TestSpace4DCycle()
	local lines = utils.ingest("day17/test.txt")
	local space = lib.Space4D:new(lines)
	lu.assertEquals(space:activeCount(), 5)
	space:cycle()
	lu.assertEquals(space:activeCount(), 29)
	space:cycle()
	lu.assertEquals(space:activeCount(), 60)
end
...
function TestSpace4DGetDimensions()
	local lines = { "#" }
	local space = lib.Space4D:new(lines)
	local _, cube = next(space.cubes)
	lu.assertNotIsNil(cube.coords.w)
	---@type CoordinateRanges
	local expected = {
		x = {
			min = -1,
...
function TestPart2()
	lu.assertEquals(lib.part2("day17/test.txt"), 848)
end
